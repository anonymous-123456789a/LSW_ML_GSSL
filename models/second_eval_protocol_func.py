...
def GetIndicator(data, latent):
    ...
    return indicator['RRE'], indicator['Trust'], indicator['RMSE'], indicator['LGD']
...
# node = torch.load('pretrained_models/cora/full_model/cora_node_level.pth')
# graph = torch.load('pretrained_models/cora/full_model/cora_graph_level.pth')

# z1 = node(cora.x, cora.edge_index)
# z2 = graph(cora.x, cora.edge_index)
...
#     return np.sqrt(sum_of_squared_differences / n**2)
# rmse(z1.detach().cpu().numpy(), z2.detach().cpu().numpy())